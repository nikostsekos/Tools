...
class PergamoSpider:
    ...
    # Use Tenacity's retry decorator with indefinite retry, exponential backoff, and wait before retry
    ...
    async def fetch_and_parse(self, url):
        ...
        try:
            response = await self.client.get(url, headers=headers, timeout=60.0)  # Set a longer timeout
            text = response.text
